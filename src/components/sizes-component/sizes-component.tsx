...
const SizesComponent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const sum = sizes?.reduce(
            (acc, { qty }) => (qty === "0" || qty === "" ? acc : acc + parseInt(qty)),
            0
        );
        setRealQty(sum!);
    ...
    const onChangeHandler = (e: any) => {
        if (e.target.name === "fabricColor")
            dispatch(secondStepActions.setFabricColor(e.target.value));
        else
            dispatch(
                secondStepActions.setSizes({ size: e.target.name, qty: e.target.value })
            );
    };
                    ...
                    return (
                        <div className={styles.size_block} key={index}>
                            <TextField
                                ...
                                onChange={onChangeHandler}
                                required
                                inputProps={{ pattern: "[0-9]{1,9999}" }}
                                placeholder='Введите размер (только цифры)'
                            />
                        </div>
                    );
                })}
                <div
                    className={isQtyEqual ? styles.size_block : styles.size_block_invalid}
                >
                    {realQty}/{textileQty}
                </div>
            </div>
        </div>
        <span className={styles.helper}>* Вводите только цифры / Для отсутствующих размеров необходимо проставить 0 / Общее количество должно совпасть</span>
        </>
    );
};
